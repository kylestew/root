...
export function easeOutInCubic(x: number): number {
    if (x < 0.5) {
        return 0.5 * (1 - Math.pow(1 - 2 * x, 3))
    } else {
        return 0.5 * Math.pow(2 * x - 1, 3) + 0.5
    }
}
...
export function easeOutInSine(x: number): number {
    if (x < 0.5) {
        x = map(x, 0, 0.5, 0, 1)
        return map(Math.sin((x * Math.PI) / 2), 0, 1, 0, 0.5) // out
    } else {
        x = map(x, 0.5, 1, 0, 1)
        return map(1 - Math.cos((x * Math.PI) / 2), 0, 1, 0.5, 1) // in
    }
}
