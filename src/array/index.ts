 ...
 * @param partial -
 */
export function partition(data: any[], size: number, step = 0, partial = false) {
    var res: any[] = []
    var n = data.length
    if (size <= 0 || step <= 0) {
        return res
 ...
 */
export function wrapSides(src: any[], numLeft = 1, numRight = 0): any[] {
 ...
 */
export function takePieces(array: any[], size: number) {
    const result = []
