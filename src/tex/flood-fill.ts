 ...
 */
export function floodFillCanvas(ctx: CanvasRenderingContext2D, startPt: Vec2, fillColor: Color, outputCtx = ctx) {
    ...
    const stack: Vec2[] = [startPt]
    const startColor: Color = getColorAtPixel(data, startPt[0], startPt[1], width)
...
function getColorAtPixel(data: Uint8ClampedArray, x: number, y: number, width: number): Color {
    const index = (y * width + x) * 4
    return [data[index], data[index + 1], data[index + 2], data[index + 3]]
}

function setColorAtPixel(data: Uint8ClampedArray, x: number, y: number, width: number, color: Color) {
    const index = (y * width + x) * 4
    data[index] = color[0]
...
function colorsMatch(color1: Color, color2: Color) {
