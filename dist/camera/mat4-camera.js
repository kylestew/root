...
const vec4TransformMat4 = (out, a, m) => {
    var [x, y, z, w] = a;
    return setn(out, m[0] * x + m[4] * y + m[8] * z + m[12] * w, m[1] * x + m[5] * y + m[9] * z + m[13] * w, m[2] * x + m[6] * y + m[10] * z + m[14] * w, m[3] * x + m[7] * y + m[11] * z + m[15] * w);
};
const setn = (out, ...args) => {
...
export function cameraProject(out, vec, viewport, combinedProjView) {
    var vX = viewport[0], vY = viewport[1], vWidth = viewport[2], vHeight = viewport[3], n = NEAR_RANGE, f = FAR_RANGE;
...
export function createOrthoFromView(out, left, right, bottom, top, zoom, near, far) {
...
export function createPerspectiveFromView(out, fov, aspect, near, far, zoom = 1, filmGauge = 35, filmOffset = 0) {
...
//# sourceMappingURL=mat4-camera.js.map